...
export default function AgregarZona() {
    ...
    const createZona = async (e) => {
        ...
        try {
            ...
            let errorMessage = 'No se pudo agregar la zona';
            if (error.response?.data?.objectError) {
                goOverErrors(error.response.data.objectError, handleSettingError);
    ...
    return (
        <Grid container spacing={2} className="max-w-[500px] m-auto p-4">
            <h2 className='flex justify-center text-xl font-bold text-blue-fond p-2 rounded-lg'>Agregar Zona</h2>
            {generalError && (
                <Grid item xs={12}>
